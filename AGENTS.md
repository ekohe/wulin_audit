...
- None of the WulinMaster assets or screens auto-load. The host app must add `//= require audit.js` to its JavaScript manifest and `*= require audit` to its stylesheet manifest. The explicit JavaScript suffix avoids the separate ESM entry. Add `item AuditLogScreen`/`item ActionLogScreen` lines to the menu-defining controller — see the README's "WulinMaster Integration" section for the full snippets.
